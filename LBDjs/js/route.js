...
function projectRouteConfig($stateProvider, $urlRouterProvider, $ocLazyLoadProvider, $locationProvider) {
    var _lazyLoad = function (loaded) {
        return function ($ocLazyLoad) {
            //默认情况下,原生的加载器会同时平行的加载你的那些文件,如果你需要按顺序加载你的文件,你可以使用参数serie: true:
            return $ocLazyLoad.load(loaded, {serie: true});
        }
    };

    $ocLazyLoadProvider.config({
        //debug: 当出现错误的时候,$ocLazyLoad返回的promise将会被rejected;
        //如果你将这个参数设置为true,$ocLazyLoad会将这些错误打印在控制台上.
        debug: false, 
        //events: 当你加载一个模块,一个组件,或者是一个文件(js/css/template)的时候;
        //$ocLazyLoad能够广播一个事件,将这个选项设置为true去激活它.
        //这些事件分别是ocLazyLoad.moduleLoaded, ocLazyLoad.moduleReloaded, ocLazyLoad.componentLoaded, ocLazyLoad.fileLoaded.
        events: true
    });
